  ...
  */
  ...
  for (int i = 0; i < n_aug_ * 2 + 1; ++i) {
    ...
    if (fabs(pd) < 0.0001) {
      ...
      x(0) += v / pd * (sin(p + pd * delta_t) - sin(p));
      x(1) += v / pd * (-cos(p + pd * delta_t) + cos(p));
    }

    x(0) += 0.5 * nua * delta_t * delta_t * cos(p);
    x(1) += 0.5 * nua * delta_t * delta_t * sin(p);
    x(2) += delta_t * nua;
    x(3) += pd * delta_t + 0.5 * nup * delta_t * delta_t;
    x(4) += delta_t * nup;
